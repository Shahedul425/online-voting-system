...
export default function AdminUploadVoters() {
    const { electionId } = useParams(); // if you mount under /admin/elections/:electionId/uploads/voters
    const [file, setFile] = useState(null);
    const [voterIdColumn, setVoterIdColumn] = useState("voterId");
    ...
    async function submit() {
        ...
        setLoading(true);
        try {
            const rep = await withUnwrap(
                OVS.uploadVoters({ electionId, voterIdColumn, emailColumn, file })
            );
            setReport(rep);
        } catch (e) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 px-4 py-10 text-white">
            <div className="mx-auto w-full max-w-2xl">
                <h1 className="text-3xl font-extrabold text-indigo-400">Upload Voters CSV</h1>
                <p className="mt-2 text-sm text-white/60">Import voters into staging and migrate on success.</p>
                ...
                <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm text-white/80">Voter ID Column</label>
                            <input
                                value={voterIdColumn}
                                onChange={(e) => setVoterIdColumn(e.target.value)}
                                className="mt-2 w-full rounded-xl border border-white/10 bg-zinc-900/60 px-4 py-3 outline-none focus:border-indigo-400/60"
                            />
                        </div>
                        <div>
                            <label className="text-sm text-white/80">Email Column</label>
                            <input
                                value={emailColumn}
